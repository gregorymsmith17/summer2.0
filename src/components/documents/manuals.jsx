...
this.handleChange = this.handleChange.bind(this);
  ...
  this.removeAuthListener = fire.auth().onAuthStateChanged(user=>{
    ...
    currentProjectRef.on('value', (snapshot) => {
      ...
      const samplesRef = fire.database().ref(`${user.uid}/${this.state.currentProject}/Manuals`);
  ...
  removeReport(itemId) {
    this.removeAuthListener = fire.auth().onAuthStateChanged(user=>{
    const sampleRef = fire.database().ref(`${this.state.userID}/${this.state.currentProject}/Manuals/${itemId}`);
    sampleRef.remove();
  });
  }
    ...
    let file = event.dataTransfer.files[0]
      ...
      uploadTask.on('state_changed', (snapshot) => {
        ...
        uploadTask.snapshot.ref.getDownloadURL().then( (downloadURL) => {
          ...
          this.setState({ downloadURL: downloadURL, documentType: 'Manuals', })
...
fillStates = (itemId) => {
  let area = '';
  this.removeAuthListener = fire.auth().onAuthStateChanged(user=>{
  const sampleRef = fire.database().ref(`${user.uid}/${this.state.currentProject}/Manuals/${itemId}`);

  let id = fire.database().ref().child(`${user.uid}/${this.state.currentProject}/Manuals/${itemId}`).key;
...
writeData = (e) => {
  ...
  this.removeAuthListener = fire.auth().onAuthStateChanged(user=>{
  const samplesRef = fire.database().ref(`${user.uid}/${this.state.currentProject}/Manuals/${this.state.id}`);
