...
this.handleChange = this.handleChange.bind(this);
  ...
  this.removeAuthListener = fire.auth().onAuthStateChanged(user=>{
    ...
    currentProjectRef.on('value', (snapshot) => {
      ...
      const samplesRef = fire.database().ref(`${user.uid}/${this.state.currentProject}/Permits`);
  ...
  removeReport(itemId) {
    this.removeAuthListener = fire.auth().onAuthStateChanged(user=>{
    const sampleRef = fire.database().ref(`${this.state.userID}/${this.state.currentProject}/Permits/${itemId}`);
    sampleRef.remove();
  });
  }
  ...
  onDrop = event => {
    ...
    let file = event.dataTransfer.files[0]
      ...
      this.removeAuthListener = fire.auth().onAuthStateChanged(user=>{
        ...
        var storageRef = firebase.storage().ref('/Permits/' + filename);
      ...
      uploadTask.on('state_changed', (snapshot) => {
        ...
        uploadTask.snapshot.ref.getDownloadURL().then( (downloadURL) => {
          ...
          this.setState({ downloadURL: downloadURL, documentType: 'Permits', })
...
fillStates = (itemId) => {
  let area = '';
  this.removeAuthListener = fire.auth().onAuthStateChanged(user=>{
  const sampleRef = fire.database().ref(`${user.uid}/${this.state.currentProject}/Permits/${itemId}`);

  let id = fire.database().ref().child(`${user.uid}/${this.state.currentProject}/Permits/${itemId}`).key;
...
writeData = (e) => {
  ...
  this.removeAuthListener = fire.auth().onAuthStateChanged(user=>{
  const samplesRef = fire.database().ref(`${user.uid}/${this.state.currentProject}/Permits/${this.state.id}`);
...
/>
        ...
        <Col xs={24} sm={24} md={24} lg={24} xl={24}>
            ...
            <Col xs={24} sm={24} md={24} lg={24} xl={24} style={{paddingTop: '20px'}}>
                ...
                <p style={{lineHeight: '2px', paddingLeft: '0px', fontSize: '32px'}}><b>UPLOAD PERMITS</b></p>
