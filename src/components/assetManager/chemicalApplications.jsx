     ...
     snapshotToArray(snapshot) {
      ...
      componentDidMount(itemId) {
        this.removeAuthListener = fire.auth().onAuthStateChanged(user=>{
          ...
          const parameterListRef = fire.database().ref(`applicationList/${user.uid}`);
          ...
          const parameterList1Ref = fire.database().ref(`applicationEvents/${user.uid}`);
...
let indexOfMaxValue = tableData2.reduce((iMax, x, i, arr) => x > arr[iMax] ? i : iMax, 0);
          ...
          const parameter2ist1Ref = fire.database().ref(`applicationEvents/${user.uid}`);
    ...
    fillStates(itemId) {
      ...
      const sample1Ref = fire.database().ref(`/applicationList/${user.uid}/${itemId}`);

      let id = fire.database().ref().child(`/applicationList/${user.uid}/${itemId}`).key;
  ...
  fillEquipment(itemId) {
    ...
    const sample1Ref = fire.database().ref(`/applicationEvents/${user.uid}/${itemId}`);

    let id = fire.database().ref().child(`/applicationEvents/${user.uid}/${itemId}`).key;
...
editParameterInfo = (e) => {
  ...
  const sampleRef = fire.database().ref(`/applicationList/${user.uid}/${this.state.id}`);
...
handleSampleChange = idx => evt => {
  ...
  sampleSubmit = (e) => {
    ...
    //fire.database().ref('samples') refers to the main title of the fire database.
    this.removeAuthListener = fire.auth().onAuthStateChanged(user=>{
    const parameterListRef = fire.database().ref(`applicationEvents/${user.uid}`);
...
parameterListRef.child(this.state.id).set(object);
   ...
   removesample(itemId) {
    ...
    const sampleRef = fire.database().ref(`/applicationEvents/${this.state.userID}/${itemId}`);
  ...
  removesample1(itemId) {
   ...
   const sampleRef = fire.database().ref(`/applicationList/${this.state.userID}/${itemId}`);
        ...
        return (
          <Layout>
            ...
            <Col xs={24} sm={24} md={18} lg={18} xl={18}>
              <h1><b>Chemical Application Manager</b></h1>
              <h3><b>{this.state.lakeName}</b></h3>
            </Col>
            <Col xs={24} sm={24} md={6} lg={6} xl={6} style={{ textAlign: 'right'}}>
          <Button size="large" type="primary" onClick={() => this.showDrawer()}>+ Add Application Event</Button>
            <Drawer
              title= "Fill in Event Form"
            ...
            <form>
              ...
              <Icon type="right-circle"  style={{fontSize: '30px'}} onClick={() => this.onClose()}>+ Add Application Event</Icon>
              </Row>
            ...
            {this.state.Parameter_List.map((parameter, idx) => {
                      ...
                      <FormGroup>
                        ...
                        <FormControl name={parameter.Maintenance_Item} type="textarea" componentClass="textarea" style={{ height: 60, width: 300}}
                          onChange={this.handleSampleChange(idx)}  placeholder="Fish" value={parameter.Maintenance_Input} /></Col>
                      </FormGroup>
                    </Row>
                    )})};
            ...
            <Button type="primary" onClick={this.sampleSubmit} bsStyle="primary">Add Event</Button>
            </Row>
            ...
            <form>
              <Row style={{textAlign: 'right'}}>
              <Icon type="right-circle"  style={{fontSize: '30px'}} onClick={() => this.onClose()}>+ Add Event</Icon>
              </Row>
            ...
            {this.state.Parameter_List.map((parameter, idx) => {
                      ...
                      <FormGroup>
                        ...
                        <Col xs={24} sm={18} md={18} lg={18} xl={18}>
                        <FormControl name={parameter.Maintenance_Item} type="textarea" componentClass="textarea" style={{ height: 60, width: 300}}
                          onChange={this.handleTableChange(idx)}  placeholder="EVENT" value={parameter.Maintenance_Input} /></Col>
                      </FormGroup>
                      </Row>
                    )})};
            ...
            <Row style={{paddingTop: '10px', textAlign: 'right'}}>
            <Button type="primary" onClick={this.sampleOverwrite} bsStyle="primary">Overwrite Event</Button>
            </Row>
            ...
            <Col xs={24} sm={24} md={24} lg={24} xl={24}>
              ...
              <TabPane tab="APPLICATION EVENTS" key="1">
            ...
            <Row style={{paddingTop: '10px'}}>
            <FormGroup>
              <Col xs={24} sm={6} md={6} lg={6} xl={6}><b>EVENT ITEMS</b></Col>
              <Col xs={24} sm={18} md={18} lg={18} xl={18}>
              <FormControl name="Maintenance_Item" onChange={this.handleChange} type="text" placeholder="Items" style={{ width: 350}} value={this.state.Maintenance_Item} /></Col>
            </FormGroup>
            </Row>
                ...
                <Col span={24}>
                  ...
                  <BootstrapTable
                    ...
                    columns={
                      ...
                      [{dataField: 'Maintenance_Item', text: 'Event Item'},
