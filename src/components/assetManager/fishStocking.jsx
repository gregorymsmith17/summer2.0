     ...
     snapshotToArray(snapshot) {
      ...
      componentDidMount(itemId) {
        this.removeAuthListener = fire.auth().onAuthStateChanged(user=>{
          ...
          const parameterListRef = fire.database().ref(`fishStockingList/${user.uid}`);
          ...
          const parameterList1Ref = fire.database().ref(`fishStockingEvents/${user.uid}`);
...
let indexOfMaxValue = tableData2.reduce((iMax, x, i, arr) => x > arr[iMax] ? i : iMax, 0);
          ...
          const parameter2ist1Ref = fire.database().ref(`fishStockingEvents/${user.uid}`);
    ...
    fillStates(itemId) {
      ...
      const sample1Ref = fire.database().ref(`/fishStockingList/${user.uid}/${itemId}`);

      let id = fire.database().ref().child(`/fishStockingList/${user.uid}/${itemId}`).key;
  ...
  fillEquipment(itemId) {
    ...
    const sample1Ref = fire.database().ref(`/fishStockingEvents/${user.uid}/${itemId}`);

    let id = fire.database().ref().child(`/fishStockingEvents/${user.uid}/${itemId}`).key;
...
editParameterInfo = (e) => {
  ...
  const sampleRef = fire.database().ref(`/fishStockingList/${user.uid}/${this.state.id}`);
...
handleSampleChange = idx => evt => {
  ...
  sampleSubmit = (e) => {
    ...
    //fire.database().ref('samples') refers to the main title of the fire database.
    this.removeAuthListener = fire.auth().onAuthStateChanged(user=>{
    const parameterListRef = fire.database().ref(`fishStockingEvents/${user.uid}`);
...
parameterListRef.child(this.state.id).set(object);
   ...
   removesample(itemId) {
    ...
    const sampleRef = fire.database().ref(`/fishStockingEvents/${this.state.userID}/${itemId}`);
  ...
  removesample1(itemId) {
   ...
   const sampleRef = fire.database().ref(`/fishStockingList/${this.state.userID}/${itemId}`);
    ...
    const cover = {
      ...
      left: '0px',
        ...
        return (
          ...
          <Button size="large" type="primary" onClick={() => this.showDrawer()}>+ Add Stocking Event</Button>
            <Drawer
              title= "Fill in Stocking Form"
            ...
            <Col xs={24} sm={24} md={24} lg={24} xl={24}>
              ...
              <TabPane tab="STOCKING EVENTS" key="1">
            ...
            <Row style={{paddingTop: '10px'}}>
            <FormGroup>
              <Col xs={24} sm={6} md={6} lg={6} xl={6}><b>STOCKING ITEMS</b></Col>
              <Col xs={24} sm={18} md={18} lg={18} xl={18}>
                ...
                <Col span={24}>
                  ...
                  <BootstrapTable
                    ...
                    columns={
                      ...
                      [{dataField: 'Maintenance_Item', text: 'Fish Item'},
