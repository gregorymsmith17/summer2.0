...
export default class uploadDrawings extends Component {
...
this.handleChange = this.handleChange.bind(this);
  ...
  deleteDrawings(row, isSelected, e, id) {

    return (
      <div style={{textAlign: 'center'}}>
      <Icon type="delete" style={{fontSize: '24px'}}
      onClick={() => this.removeDrawings(`${isSelected.id}`)}>
        Click me
      </Icon>
      </div>
  ...
  onDrop = event => {
    ...
    let file = event.dataTransfer.files[0]
      ...
      this.removeAuthListener = fire.auth().onAuthStateChanged(user=>{
        ...
        var storageRef = firebase.storage().ref('/Drawings/' + filename);
...
fillStates = (itemId) => {
  ...
  let id = fire.database().ref().child(`/Drawings/${user.uid}/${itemId}`).key;
...
writeData = (e) => {
  ...
  samplesRef.child(this.state.id).set(metaDataReport);
...
onClose = () => {
  ...
  render() {
    ...
    <Button type="primary" onClick={this.writeData} bsStyle="primary">Edit Drawing Name</Button>
    </Row>
...
/>
        ...
        <Col xs={24} sm={24} md={24} lg={24} xl={24}>
            ...
            <Col xs={24} sm={24} md={24} lg={24} xl={24} style={{paddingTop: '20px'}}>
                ...
                <p style={{lineHeight: '2px', paddingLeft: '0px', fontSize: '32px'}}><b>UPLOAD DRAWINGS</b></p>
...
<div style={{
    ...
    <TableHeaderColumn width='80px'
          ...
          <TableHeaderColumn width='80px'
            dataField='button'
            dataFormat={this.deleteDrawings.bind(this)}
