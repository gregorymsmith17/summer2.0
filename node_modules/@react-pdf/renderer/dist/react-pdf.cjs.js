...
class Text$1 extends Base {
  ...
  layoutText(width, height) {
    ...
    if (!this._container || this.props.render) {
      const path$$1 = new textkitCore.Path().rect(0, 0, width, containerHeight);
      const container = new textkitCore.Container(path$$1);
...
class Image$1 extends Base {
  ...
  renderImage() {
    ...
    if (this.image.data) {
      const { width, height, xOffset, yOffset } = resolveObjectFit(this.props.style.objectFit, this.width - padding.left - padding.right, this.height - padding.top - padding.bottom, this.image.width, this.image.height, objectPositionX, objectPositionY);

      if (width !== 0 && height !== 0) {
        this.root.instance.image(this.image.data, left + padding.left + xOffset, top + padding.top + yOffset, { width, height });
      } else {
